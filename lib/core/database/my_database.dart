...
import 'package:arae_blueprint/generated/l10n.dart';
import 'package:arae_blueprint/src/model/article.dart';
import 'package:arae_blueprint/src/modules/app_language/model/app_language.dart';
import 'package:arae_blueprint/src/modules/app_language/services/app_language_service.dart';

import 'package:arae_blueprint/src/modules/tag/model/tag.dart';
...
class MyDatabase {
  ...
  static void initialize(BuildContext context) {
    initializeImageUrls(context);
    currentDB = _articles;
  }

  static void initializeImageUrls(BuildContext context, {int quality=70}) {
    List<Article> output = [];

    double width = MediaQuery.of(context).size.width;
    width = width * 1.8;

    developer.log('Inicializando MyDatabase Urls con width: ${width.round}',
  ...
  static void loadCurrentDB(AppLanguage languageSelected) {
    List<Article> output = [];

    if (languageSelected != null) {
      _articles.map((a) {
        if (a.language.compareTo(languageSelected.value) == 0) output.add(a);
      }).toList();
      ...
      developer.log('WARNING: languagesSelected es null',
          name: 'pro.arae.MyDatabase.loadCurrentDB');

      loadCurrentDB(AppLanguageService.defaultAppLanguage);
    }
  }
  ...
  static List<Article> _articles = [
    ...
    Article.admin(
        ...
        subtitle:
            'Quiere decir que hay que relacionarse con quien sea tu igual.'),
    Article.admin(
        id: 156,
        language: 'en',
    ...
    Article.admin(
        ...
        subtitle:
            'This means that feelings of hurt will leave as time passes by',
        tags: [Tag.newest],
        image:
